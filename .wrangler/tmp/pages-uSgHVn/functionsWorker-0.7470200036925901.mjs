...
// ../.wrangler/tmp/bundle-J7xvMj/checked-fetch.js
...
// ../.wrangler/tmp/bundle-J7xvMj/strip-cf-connecting-ip-header.js
...
async function onRequestGet(context) {
  ...
  for (const path of possiblePaths) {
    try {
      const response = await fetch(`${path}?v=${(/* @__PURE__ */ new Date()).getTime()}`);
      if (response.ok) {
        const data = await response.json();
        let examen = Array.isArray(data) ? {
          materia: "Examen Gen\xE9rico",
          titulo: sanitizedId,
          duracion: 0,
...
async function onRequestGet2(context) {
  ...
  for (const path of possiblePaths) {
    try {
      const finalUrl = `${path}?v=${(/* @__PURE__ */ new Date()).getTime()}`;
  ...
  return new Response(JSON.stringify({
    error: "No se pudo cargar el cat\xE1logo.",
    details: lastError,
    tried: possiblePaths
  }), {
    status: 500,
...
// ../.wrangler/tmp/pages-uSgHVn/functionsRoutes-0.49649825785593116.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-J7xvMj/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-J7xvMj/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7470200036925901.mjs.map
